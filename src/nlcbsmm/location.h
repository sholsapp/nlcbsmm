...
namespace NLCBSMM {
    ...
    class NetworkLocation : public Location {
        ...
        private:
            ...
            void fetchNetworkPage() {
                ...
                int read = recvfrom(socket_num, recbuf, bufsize, NULL, (struct sockaddr*) &host, &fromlen);
    ...
    class GZIPLocation : public Location {
    public:
      GZIPLocation() {
        actualPage = mmap(NULL, PAGESIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
        sz = 4096;
      ...
      virtual void* getPage() {
        char  result [PAGESIZE];
        //return NULL; 
        uncompress(result, &PAGESIZE, actualPage, PAGESIZE);
        return actualPage;
      }
      ...
      void setPage(void* page) {
        ...
        compress(actualPage, &PAGESIZE, page, PAGESIZE);
        //memcpy(actualPage, page, PAGESIZE);
        fprintf(stderr, "Zeroing page at %p\n", page);
        //make that page unreadable      
	memset(page, 0, 4096);

        //shouldn't this page be freed? 
        return;
      }
    private:
      void * actualPage;
